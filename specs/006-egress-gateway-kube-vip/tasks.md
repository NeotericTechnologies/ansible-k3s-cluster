...
- [X] T021 [US1] Create `ansible/roles/kube-vip/templates/kube-vip-egress-policy.yaml.j2` — `CiliumEgressGatewayPolicy` CR with `egressGateway.egressIP: {{ kube_vip_egress_ip }}`, `egressGateway.nodeSelector: {{ kube_vip_egress_gateway_node_selector }}`, `egressGateway.interface: {{ kube_vip_egress_gateway_interface }}`, `destinationCIDRs: {{ kube_vip_egress_destination_cidrs }}`, and `selectors` entries using `podSelector: {{ kube_vip_egress_pod_selector }}` plus optional `namespaceSelector: {{ kube_vip_egress_namespace_selector }}`. REVISED: moved to `ansible/roles/cilium/templates/cilium-egress-gateway-policy.yaml.j2` — it is a Cilium CRD and the cilium role runs after kube-vip in `cluster-core.yml`, so applying it from kube-vip would fail (CRD not yet registered).
- [X] T022 [US1] Add egress gateway tasks to `ansible/roles/kube-vip/tasks/install.yml` — `kubernetes.core.k8s` with `state: present` for egress Service and CiliumEgressGatewayPolicy, both gated on `kube_vip_egress_enabled | bool`; force `kube_vip_svc_election_enabled: true` when egress enabled (set_fact). REVISED: the CiliumEgressGatewayPolicy apply task lives in `ansible/roles/cilium/tasks/install.yml` (after the Cilium DaemonSet-ready wait) instead — kube-vip's `install.yml` only applies the egress Service.
...
- [X] T034 [P] Add Cilium to `ansible/playbooks/cluster-addons.yml` \u2014 REVISED: Cilium is a core cluster component (CNI), not an optional add-on, so no references were added to `cluster-addons.yml`. Deployment, version assertion, and status reporting for Cilium live entirely in `cluster-core.yml` (T033), consistent with how `k3s-server`/`k3s-agent` are treated as core rather than add-ons.
