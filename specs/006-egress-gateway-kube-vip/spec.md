...
- **FR-003b**: Cilium MUST be the active CNI for the cluster; the playbook MUST fail with a descriptive error if the `cilium_enabled` variable is not `true` when `kube_vip_egress_enabled: true`. Verifying Cilium is actually running in the live cluster (vs. the variable being set) is out of scope for pre-flight validation.
